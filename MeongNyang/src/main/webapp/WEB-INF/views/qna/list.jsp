	...
	$(function(){
		...
		$("#btnDeleteList").click(function(){
			...
			if(confirm("선택한 게시물을 삭제 하시겠습니까 ?")) {
				const f = document.listForm;
				f.action="${pageContext.request.contextPath}/qna/deleteList.do";
				f.submit();
			}
		});
	});
...
<main>
	<div class="container body-container">
	    <div class="body-title">
			<h2><i class="fas fa-clipboard-list"></i> 1:1 문의 </h2>
	    </div>
					...
					<tbody>
						...
						<c:forEach var="dto" items="${list}" varStatus="status">
							<tr>
								...
								<td class="left">
									<a href="${articleUrl}&num=${dto.num}">${dto.subject}</a>
									<c:if test="${dto.gap<1}"><img src="${pageContext.request.contextPath}/qna/images/new.gif"></c:if>
								</td>
								<td>${dto.userName}</td>
								<td>${dto.reg_date}</td>
								<td>${dto.hitCount}</td>
			...
			<table class="table">
				<tr>
					...
					<td align="right" width="100">
						<c:if test="${sessionScope.member.userId=='admin'}">
							<button type="button" class="btn" onclick="location.href='${pageContext.request.contextPath}/qna/write.do?size=${size}';">글올리기</button>
						</c:if>
					</td>
				</tr>
			</table>
